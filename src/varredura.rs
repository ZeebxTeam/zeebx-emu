...
pub fn examina(arquivo: &Path, ms_virtuais: u32, teto: Duration) -> Relatorio {
    ...
    let mut session = match Session::start_with(arquivo, crate::app::PORTAS_PADRAO, None, false, None, Default::default()) {
        Ok(session) => session,
...
pub fn abre(arquivo: &Path) -> Result<Duration, String> {
    let comeco = Instant::now();
    match Session::start_with(arquivo, crate::app::PORTAS_PADRAO, None, false, None, Default::default()) {
